...
const CatererSearch = () => {
  ...
  const fetchNearbyCaterers = async (lat, lng) => {
    try {
      const URL = `http://localhost:3000/api/caterer/nearby?lat=${lat}&lng=${lng}&radius=${radius}`;
  ...
  const handleSearch = () => {
    if (currentLocation) {
      fetchNearbyCaterers(currentLocation.lat, currentLocation.lng);
    } else {
      // If no location is set, use a default location or show an error
      console.error("No location set for search");
    }
  };
